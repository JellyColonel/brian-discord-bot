...
class Admin(commands.Cog):
    ...
    async def purge(
        self, 
        inter,
        amount: int = commands.Param(description="Number of messages to delete", ge=1, le=100),
        user: disnake.User = commands.Param(description="Filter by user", default=None)
    ):
        ...
        try:
            ...
            logger.info(f"{inter.author} purged {len(deleted)} messages in #{inter.channel.name}")
    ...
    async def kick(
        ...
        reason: str = commands.Param(description="Reason for kick", default="No reason provided")
    ):
    ...
    async def ban(
        ...
        reason: str = commands.Param(description="Reason for ban", default="No reason provided"),
        delete_days: int = commands.Param(description="Days of messages to delete", default=1, ge=0, le=7)
    ):
    ...
    async def timeout(
        ...
        duration: int = commands.Param(description="Duration in minutes", ge=1, le=40320),  # Max 28 days (40320 minutes)
        reason: str = commands.Param(description="Reason for timeout", default="No reason provided")
    ):
        ...
        try:
            ...
            time_str = format_timespan(duration * 60)  # Convert to seconds for the helper
    ...
    async def reload(
        self,
        inter,
        extension: str = commands.Param(description="Extension to reload (or 'all')")
    ):
